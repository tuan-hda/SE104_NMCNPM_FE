...
const StoreLocator = () => {
  ...
  // Get open / close time
  const getOpenTime = branch =>
    branch.openData.fromHour +
    ':' +
    (branch.openData.fromMin === 0 ? '00' : branch.openData.fromMin)
  const getCloseTime = branch =>
    branch.openData.toHour +
    ':' +
    (branch.openData.toMin === 0 ? '00' : branch.openData.toMin)

  return loading ? null : (
    <div className='-mt-28 relative text-sm'>
      <MapContainer
        center={getLocation(branches[0])}
        zoom={13}
      ...
      {/* Menu */}
      <div
        className='absolute top-24 left-3 p-2 bg-white rounded-md z-10 cursor-pointer hover:bg-gray-200 transition duration-300'
        onClick={() => toggle(!isShowing)}
      >
        <img alt='Menu Icon' src={Menu} className='w-5 h-5' />
      </div>

      {/* Back button */}
      <div
        className={`absolute left-3 bottom-4 top-24 bg-white bg-opacity-90 rounded-lg w-80 p-4 ${
          isShowing ? '' : '-translate-x-[120%]'
        ...
        <div className='mt-7 h-5/6 overflow-auto'>
          {branches.map((branch, index) => (
            <div key={index}>
              {/* Address */}
              <p
                className='p-4 cursor-pointer hover:bg-gray-300 rounded-md transition duration-300'
              ...
              {index !== branches.length - 1 && (
                <p className='border-t-[1px] w-full my-2' />
              )}
            </div>
          ))}
        </div>
